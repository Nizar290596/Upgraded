...
for lpC = LoopClow : LoopCup
    ...
    %for fragementation, start from sec3  
        if isAit1 && SecCoeffs(ri,7) == -1 && (nCp > nC(1)*2)%is product Ait1? and fragmentation
            ...
            %Asp = strcat('Ai_',num2str(nCintvl-1),'_',num2str(nHintvl-1))
        elseif isAjt1 && SecCoeffs(ri,7) == -1 && (nCp > nC(1)*2)%is product Ajt1 and fragmentation
    ...
    if nCp <= nC(end) && (nCp >= nHp)
        ...
        if SecCoeffs(ri,1) == -1 %for coagulation
            ...
            Gamma = CoagGamma_1Dck(ReactantNC(1),ReactantNC(2),ReactantNH(1),ReactantNH(2));
