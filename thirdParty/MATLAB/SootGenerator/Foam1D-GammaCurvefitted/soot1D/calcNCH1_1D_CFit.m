...
for lpC = LoopClow : LoopCup
    ...
    if (nCp > nC(1)) && (nCp <= nC(end))
        ...
        if isAit1 && (nHp <= nCp) %is product Ait1
        ...
        elseif isAjt1 && (nHp <= (nCp-1))%is product Ajt1
            [pC1,pC2,pC1coeff,pC2coeff] = findVertices1D(nCp,nC);
            PrdtType = 'Aj';
    ...
    elseif nCp<=nC(1) 
            %for nCp<=nC1, (nHp <= (nH(1)-2)/nC(1)*nCp + 2) is in polygon
        if isAit1 
            ...
            buildReaction1_1D;%build reaction
        elseif isAjt1 
            pC1=0;pC2=1;pC1coeff=0;pC2coeff=nCp/nC(1);
            PrdtType = {'Aj'};
    ...
    if nCp <= nC(end) && (nCp >= nHp)
        ...
        if SecCoeffs(ri,1) == -1 %for coagulation
            ...
            coeffA = CoagBeta_1D(ReactantNC(1),ReactantNC(2),ReactantNH(1),ReactantNH(2));
            
            GammaArrConst{ri}(LPi,1:4) = CoagGamma_1D_CFit(ReactantNC(1),ReactantNC(2),ReactantNH(1),ReactantNH(2));

            GammaArrConst{ri}(LPi,5) = CoagGamma_1D(ReactantNC(1),ReactantNC(2),ReactantNH(1),ReactantNH(2));
