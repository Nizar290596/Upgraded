...
function GammaConst = CoagGamma_1Dck_CFit(numC1,numC2,numH1,numH2)
...
if((numC1+numC2) >= (48965+19586)) %>bin11+bin10
    Tstart = 50;
else
    Tstart = 30;
end
...
if(numC1<96 || numC2<96) %>bin11+bin10
    CFtolerance = 1e-10;
else
    CFtolerance = 1e-10;
end
...
options = optimoptions(@lsqnonlin,'MaxFunctionEvaluations',1000,'Display','off',...
                'FunctionTolerance',CFtolerance,'OptimalityTolerance',CFtolerance);
            
CFitConst = lsqnonlin(@fit_diff,X0,[],[],options,X,Y);
            

GammaConst =[CFitConst;phi0];
