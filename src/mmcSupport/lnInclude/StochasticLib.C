   ...
   if (n < FAK_LEN) {
      if (n <= 1) {
         if (n < 0) 
            ...
            (
                "StochasticLib.C"
            )   << "Parameter negative in LnFac function.\n" << exit(FatalError);
...
int32_t StochasticLib1::Hypergeometric (int32_t n, int32_t m, int32_t N) {
   ...
   */   
   ...
   if (n > N || m > N || n < 0 || m < 0)
            ...
            (
                "StochasticLib.C"
            )   << "Parameter out of range in hypergeometric function.\n" << exit(FatalError);
...
int32_t StochasticLib1::HypInversionMod (int32_t n, int32_t m, int32_t N) {
   ...
   */
   ...
   Mp = static_cast<double>(m + 1);
   np = static_cast<double>(n + 1);
   ...
   if (N != hyp_N_last || m != hyp_m_last || n != hyp_n_last) {
      ...
      hyp_mode = static_cast<int32_t>(modef);            // mode, integer
      ...
      // bound = min(n, static_cast<int32_t>(modef + k*c'))
      hyp_bound = static_cast<int32_t>(modef + 11. * sqrt(modef * (1.-p) * (1.-n/static_cast<double>(N))+1.));
...
int32_t StochasticLib1::HypRatioOfUnifoms (int32_t n, int32_t m, int32_t N) {
   ...
   */
   ...
   if (hyp_N_last != N || hyp_m_last != m || hyp_n_last != n) {
      ...
      rNN = 1. / (static_cast<double>(N)*(N+2));                             // make two divisions in one
      my = static_cast<double>(n) * m * rNN * (N+2);                         // mean = n*m/N
      mode = static_cast<int32_t>(double(n+1) * double(m+1) * rNN * N);    // mode = floor((n+1)*(m+1)/(N+2))
      var = static_cast<double>(n) * m * (N-m) * (N-n) / (static_cast<double>(N)*N*(N-1));// variance
      hyp_h = sqrt(SHAT1 * (var+0.5)) + SHAT2;                  // hat width
      hyp_a = my + 0.5;                                         // hat center
      hyp_fm = fc_lnpk(mode, L, m, n);                          // maximum
      hyp_bound = static_cast<int32_t>(hyp_a + 4.0 * hyp_h);               // safety-bound
      if (hyp_bound > n) hyp_bound = n;
   }    
   while(1) {
...
int32_t StochasticLib1::PoissonRatioUniforms(double L) {
   ...
   */
   ...
   if (pois_L_last != L) {
      ...
      int32_t mode = static_cast<int32_t>(L);                      // mode
      pois_g  = log(L);
      pois_f0 = mode * pois_g - LnFac(mode);          // value at mode
      pois_h = sqrt(SHAT1 * (L+0.5)) + SHAT2;         // hat width
      pois_bound = static_cast<int32_t>(pois_a + 6.0 * pois_h);  // safety-bound
   }
   while(1) {
      u = Random();
      if (u == 0) continue;                           // avoid division by 0
   ...
   */
   ...
   if (n <= 0 || p <= 0) {
      ...
      // error exit
            ...
            (
                "StochasticLib.C"
            )   << "Parameter out of range in binomial function.\n" << exit(FatalError);
   }
...
int32_t StochasticLib1::BinomialInver (int32_t n, double p) {
   ...
   */   
   ...
   rc = (n + 1) * p;
   bound = static_cast<int32_t>(rc + 11.0*(sqrt(rc) + 1.0));
   if (bound > n) bound = n; 
   q = p / (1. - p);
...
int32_t StochasticLib1::BinomialRatioOfUniforms (int32_t n, double p) {
   ...
   */   
   ...
   while (1) {                                   // rejection loop
      ...
      k = static_cast<int32_t>(x);                            // truncate
...
void StochasticLib1::Multinomial (int32_t * destination, double * source, int32_t n, int colors) {
   ...
   */
   ...
   if (n < 0 || colors < 0) 
            ...
            (
                "StochasticLib.C"
            )   << "Parameter negative in multinomial function.\n" << exit(FatalError);
   ...
   if (sum == 0 && n > 0) 
            ...
            (
                "StochasticLib.C"
            )   << "Zero sum in multinomial function.\n" << exit(FatalError);
...
void StochasticLib1::Multinomial (int32_t * destination, int32_t * source, int32_t n, int colors) {
   ...
   for (i=0; i<colors-1; i++) { 
      ...
      p = source[i];
      x = Binomial(n, static_cast<double>(p)/sum);
      n -= x; sum -= p;
      destination[i] = x;
   }
   // get the last one
...
double StochasticLib1::NormalTrunc(double m, double s, double limit) {
   ...
   if (limit < s) 
            ...
            (
                "StochasticLib.C"
            )   << "Limit out of range in NormalTrunc fuction.\n" << exit(FatalError);
...
int StochasticLib1::Bernoulli(double p) {
   ...
   if (p < 0 || p > 1) 
            ...
            (
                "StochasticLib.C"
            )   << "Parameter out of range in Bernoulli function.\n" << exit(FatalError);
...
double StochasticLib1::ClippedGaussian(double m, double s, double lLimit, double uLimit) {
   // Truncated normal distribution
   // The tails are cut off so that the output
   // is in the interval from (m-limit) to (m+limit)

    if (m == 1.0 || m == 0.0 || s == 0.0)
    {
//        if (m == 1.0)
//        {
//            m = m - SMALL;        
//        }
    ...
    {
        if (lLimit > m || uLimit < m) 
                FatalErrorIn
                (
                    "StochasticLib.C"
                )   << "Limit out of range in ClippedGaussian fuction.\n" << exit(FatalError);

    //- The idea of ClippedGaussian distribution is obtained from Matthew Cleary's PhD thesis.
        //- some initializations
        double meanG = m; 
        double g = min(pow(s, 2.0), min (pow((0.5*m), 2.0), pow((0.5-0.5*m), 2.0))); 

        double alpha1 = (1.0 - m) * (1.0 - (m * (1.0 - m) - pow(s, 2.0))/(m * (1.0 - m) - 0.777 * g));
        double alpha2 = m * (1.0 - (m * (1.0 - m) - pow(s, 2.0))/(m * (1.0 - m) - 0.777 * g));
        ...
        if (mu < alpha1)
        {
//Info<< "\n ClippedGaussian lower limit reached" << endl;
            x = lLimit;
//Info<< "\n       x = " << x << endl;
        }
        else if (mu > (1.0 - alpha2))
        {
//Info<< "\n ClippedGaussian upper limit reached" << endl;
            x = uLimit;
//Info<< "\n       x = " << x << endl;
        }
        else
        {    
//Info<< "\n ClippedGaussian middle range" << endl;
            do {
                x = Normal(meanG, sqrt(g));
               } while (fabs((x-meanG)/sqrt(g)) > 2.0);
//Info<< "\n       x = " << x << endl;
        }
