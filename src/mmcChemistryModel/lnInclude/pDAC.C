...
Foam::particleChemistryReductionMethods::pDAC<ReactionThermo, ThermoType>::pDAC
(
    const IOdictionary& dict,
    particleTDACChemistryModel<ReactionThermo, ThermoType>& chemistry
)
:
    particleChemistryReductionMethod<ReactionThermo, ThermoType>(dict, chemistry),
...
template<class ReactionThermo, class ThermoType>
void Foam::particleChemistryReductionMethods::pDAC<ReactionThermo, ThermoType>::reduceMechanism
(
    const scalarField &c,
    const scalar T,
    const scalar p
)
{
    ...
    scalarField PA(this->nSpecie_,0.0); //production of A
    scalarField CA(this->nSpecie_,0.0); //consumption of A
