...
Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::pISAT
(
    const dictionary& chemistryProperties,
    particleTDACChemistryModel<ReactionThermo, ThermoType>& chemistry
)
:
    particleChemistryTabulationMethod<ReactionThermo, ThermoType>
    (
        chemistryProperties,
        chemistry
    ),
    ...
    KeepIfFull_(this->coeffsDict_.lookupOrDefault("KeepIfFull", true)),
    nRetrieved_(0),
    nGrowth_(0),
    nAdd_(0),
    nDI_(0),
    cleaningRequired_(false),
    listNleaves_(chPMaxLifeTime_),
    listNretrieves_(chPMaxLifeTime_),
    TestSwitch_
    (
        this->coeffsDict_.lookupOrDefault("TestSwitch", false)
    ),
    listNGrowRetrieves_(maxGrowth_+1),
    listNGrowBad_(maxGrowth_+1),
    SumGlobalError_(0),
    MaxGlobalError_(0)
{
    if (this->active_)
    ...
    if (this->log())
    {
        ...
        ISATlistNleaves_ = chemistry.logFile("ISATlistNleaves.out");
        ISATlistNretrieves_ = chemistry.logFile("ISATlistNretrieves.out");

        ISATlistNGrowRetrieves_ = chemistry.logFile("ISATlistNGrowRetrieves.out");
        ISATlistNGrowBad_ = chemistry.logFile("ISATlistNGrowBad.out");

        summaryFile_ = chemistry.logFile("Summary_isat.out");
        summaryFile_() 
            << "Time" << "    " << "nRetrieved"
            << "    " << "nGrowth" << "    " << "nAdd" << "    " << "nDI"
            << "    " << "Size" << "    "<< "SumGlobalError" << "    " << "MaxGlobalError_"<< endl;
    }
    listNleaves_ = 0;
    listNretrieves_ = 0;
    listNGrowRetrieves_ = 0;
    listNGrowBad_ = 0;
}
...
template<class ReactionThermo, class ThermoType>
void Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::addToMRU
(
    pChemPointISAT<ReactionThermo, ThermoType>* phi0
)
{
    if (maxMRUSize_ > 0 && MRURetrieve_)
    {
        ...
        bool isInList = false;
        typename SLList <pChemPointISAT<ReactionThermo, ThermoType>*>::iterator iter =
            MRUList_.begin();
...
template<class ReactionThermo, class ThermoType>
void Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::calcNewC
(
    pChemPointISAT<ReactionThermo, ThermoType>* phi0,
...
template<class ReactionThermo, class ThermoType>
bool Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::grow
(
    pChemPointISAT<ReactionThermo, ThermoType>* phi0,
    const scalarField& phiq,
...
template<class ReactionThermo, class ThermoType>
bool
Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::cleanAndBalance()
{
    bool treeModified(false);


//    scalar b4clean = this->chemistry_.mesh().time().elapsedCpuTime();
    ...
    pChemPointISAT<ReactionThermo, ThermoType>* x = chemisTree_.treeMin();
    while(x != nullptr)
    {
        pChemPointISAT<ReactionThermo, ThermoType>* xtmp =
            chemisTree_.treeSuccessor(x);

        scalar elapsedTimeSteps = this->chemistry_.timeSteps() - x->lastTimeUsed();

        //if ((elapsedTimeSteps > chPMaxLifeTime_) || (x->nGrowth() > maxGrowth_))
        if (elapsedTimeSteps > chPMaxLifeTime_)
        {
...
//    scalar Afclean = this->chemistry_.mesh().time().elapsedCpuTime();
...
//    scalar Afbaltree = this->chemistry_.mesh().time().elapsedCpuTime();

//    Pout << "clean tree takes " << Afclean - b4clean
//        << " s, blance tree takes " << Afbaltree - Afclean << endl;
...
template<class ReactionThermo, class ThermoType>
void Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::computeA
(
...
template<class ReactionThermo, class ThermoType>
bool Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::retrieve
(
...
)
{
    ...
    if (chemisTree_.size())
    {
        ...
        else if (MRURetrieve_)
        {
            typename SLList
            <
                pChemPointISAT<ReactionThermo, ThermoType>*
    ...
    if (retrieved)
    {
        ...
        scalar elapsedTimeSteps =
            this->chemistry_.timeSteps() - phi0->lastTimeUsed();
        ...
        //phi0->lastTimeUsed() = this->chemistry_.timeSteps();// update the leaf actually used, not modified now
        addToMRU(phi0);
        calcNewC(phi0,phiq, Rphiq);
        nRetrieved_++;
        listNretrieves_[phi0->TimeNotUsed()]++; //for ISAT testing of life
        if(Pstream::myProcNo() == 6)
        {
        //Pout << "Retrieved from leafID: " << phi0->leafID_ << endl;
        //Pout << "leaf composition: " << phi0->phi() << endl;
        //Pout << "query composition: " << phiq << endl;
        //Pout << "mapping composition: " << phi0->Rphi() << endl;
        //Pout << "mapping composition: " << Rphiq << endl;
        //Pout << "listNretrieves_[phi0->TimeNotUsed()]++" << endl;
        //Pout << "sum of listNretrieves_ is: " << nRetrieved_ << endl;
        }
        return true;
    }
    else
...
template<class ReactionThermo, class ThermoType>
Foam::label Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::add
(
    const scalarField& phiq,
    const scalarField& Rphiq,
    const scalar rho,
...
)
{
    ...
    if (chemisTree().isFull() && KeepIfFull_)
    {
        return growthOrAddFlag; //should be other indicator
    }
    ...
    if (chemisTree().isFull() && !KeepIfFull_)
    {
        ...
        if (!cleanAndBalance())
        {
            DynamicList<pChemPointISAT<ReactionThermo, ThermoType>*> tempList;
            if (maxMRUSize_>0)
            {
                // Create a copy of each pChemPointISAT of the MRUList_ before
                // they are deleted
                typename SLList
                <
                    pChemPointISAT<ReactionThermo, ThermoType>*
                ...
                for ( ; iter != MRUList_.end(); ++iter)
                {
                    tempList.append
                    (
                        new pChemPointISAT<ReactionThermo, ThermoType>(*iter())
                    );
                }
            }
            chemisTree().clear();
            ...
            pChemPointISAT<ReactionThermo, ThermoType>* nulPhi = 0;
            forAll(tempList, i)
            {
    ...
    (
        ...
        this->tolerance(),
        scaleFactor_.size(),
        lastSearch_ // lastSearch_ may be nullptr (handled by pBinaryTree)
    );
...
template<class ReactionThermo, class ThermoType>
void
Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::writePerformance()
{
    if (this->log())
    {
        nRetrievedFile_()
        ...
        summaryFile_() 
            << runTime_.timeOutputValue() << "    " << nRetrieved_
            << "    " << nGrowth_ << "    " << nAdd_ << "    " << nDI_
            << "    " << this->size() << "    " << SumGlobalError_ 
            << "    " << MaxGlobalError_ << endl;
...
template<class ReactionThermo, class ThermoType>
void
Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::ISATLifeStatistics()
{

    pChemPointISAT<ReactionThermo, ThermoType>* x = chemisTree_.treeMin();
    while(x != nullptr)
    {
        listNleaves_[x->TimeNotUsed()]++;

        x->TimeNotUsed() = this->chemistry_.timeSteps() - x->lastTimeUsed();

        x = chemisTree_.treeSuccessor(x);
    }

        ISATlistNleaves_()<< runTime_.timeOutputValue();
        forAll(listNleaves_,i)
        {
        ISATlistNleaves_() << "  " << listNleaves_[i];
        }
        ISATlistNleaves_()<< endl;

        ISATlistNretrieves_() << runTime_.timeOutputValue();
        forAll(listNretrieves_,i)
        {
        ISATlistNretrieves_() << "  " << listNretrieves_[i];
        }
        ISATlistNretrieves_() << endl;

    listNleaves_ = 0;
    listNretrieves_ = 0;

        ISATlistNGrowRetrieves_()<< runTime_.timeOutputValue();
        forAll(listNGrowRetrieves_,i)
        {
        ISATlistNGrowRetrieves_() << "  " << listNGrowRetrieves_[i];
        }
        ISATlistNGrowRetrieves_()<< endl;

        ISATlistNGrowBad_() << runTime_.timeOutputValue();
        forAll(listNGrowBad_,i)
        {
        ISATlistNGrowBad_() << "  " << listNGrowBad_[i];
        }
        ISATlistNGrowBad_() << endl;
...
template<class ReactionThermo, class ThermoType>
bool
Foam::chemistryTabulationMethods::pISAT<ReactionThermo, ThermoType>::CheckRetrieveSolution
(
    const scalarField& phiq,
    const scalarField& DIRphiq
) 
{
    bool inTolc = lastSearch_->checkSolution(phiq, DIRphiq);
    
    scalar error = lastSearch_->checkSolutionError_;
    SumGlobalError_ += error;
    if(error > MaxGlobalError_)
        MaxGlobalError_ = error;

//        if(Pstream::myProcNo() == 6)
//            Pout << "before increment: " << listNGrowRetrieves_[lastSearch_->nGrowth()] << endl;

    listNGrowRetrieves_[lastSearch_->nGrowth()]++;

//        if(Pstream::myProcNo() == 6)
//        {
//        Pout << "listNGrowRetrieves_[lastSearch_->nGrowth()]++" << endl;
//        Pout << "nGrowth = " << lastSearch_->nGrowth() << endl;
//        Pout << "after increment: " << listNGrowRetrieves_[lastSearch_->nGrowth()] << endl;
//        label sum = 0;
//            forAll(listNGrowRetrieves_,i)
//            {
//            sum += listNGrowRetrieves_[i];
//            }
//        Pout << "sum of listNGrowRetrieves_ is: " << sum << endl;
//        }

    if(!inTolc)
    {
        listNGrowBad_[lastSearch_->nGrowth()]++;

        //Pout << lastSearch_->nGrowth() << endl;

//        if(lastSearch_->nGrowth() == 0)
//        {
//        if(Pstream::myProcNo() == 6)
//        {
//        std::cout << this->size() << ": "<<lastSearch_->leafID_ <<"th leaf have error: "
//                     << lastSearch_->checkSolutionError_ << "\n"<< std::endl;
