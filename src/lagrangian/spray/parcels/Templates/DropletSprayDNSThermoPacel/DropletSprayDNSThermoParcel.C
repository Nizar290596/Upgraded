...
void Foam::DropletSprayDNSThermoParcel<ParcelType>::setCellValues
(
...
)
{
    ...
    td.pc() = td.pInterp().interpolate(this->coordinates(), tetIs);
...
template<class ParcelType>
template<class TrackCloudType>
void Foam::DropletSprayDNSThermoParcel<ParcelType>::cellValueSourceCorrection
(
    TrackCloudType& cloud,
    trackingData& td,
    const scalar dt
...
template<class ParcelType>
template<class TrackCloudType>
void Foam::DropletSprayDNSThermoParcel<ParcelType>::calcReferenceValues
(
    TrackCloudType& cloud,
...
{
    const scalar A = cloud.refPropertiesFactor();
...
template<class ParcelType>
template<class TrackCloudType>
void Foam::DropletSprayDNSThermoParcel<ParcelType>::calc
(
    TrackCloudType& cloud,
    trackingData& td,
    const scalar dt
)
{
    ...
    const scalar mD0 = this->mass0_;        // Note: Mass is calculated based on density and volume()!
    const scalar T0 = this->T_;

    // Infinity conditions
    scalar TInf=0;
    scalar pInf=0;
    scalarList YInf(cloud.thermo().carrier().species().size());

    const speciesTable N = cloud.thermo().carrier().species();
    
    forAll(N,i)
    {
        YInf[i] = td.YiInterp(N[i]).interpolate(this->coordinates(), this->currentTetIndices());
    }
    ...
    // Sotre the infinity conditions
    TInf_Euler_ = TInf;
    YInf_Euler_ = YInf[jFuel_];

    switch (cloud.integrationScheme())
    {
        case DropletSprayDNSThermo::integrationScheme::EulerExplicit:
        ...
        case DropletSprayDNSThermo::integrationScheme::CrankNicolson:
    ...
    // Get coupling model to MMC cloud
    auto& couplingModel = cloud.dropletToMMCCoupling();

    // Store the infinity conditions of the stochastic particle
    scalar TInf_SP=0;
    scalar pInf_SP=0;
    scalarList YInf_SP(cloud.thermo().carrier().species().size());
    ...
    if (cloud.solution().coupled() && this->active())
    {
        ...
        const scalar hvap = hA_new+cloud.liquidProperties().Lv(pInf,this->T_);
        const scalar totdhliquid = -(mDNew*hA_new - mD0*hA_old);
        const scalar dhvapor = -(mDNew - mD0)*(hvap);
        ...
        dhsTrans =  totdhliquid - dhvapor; 
               
        // Update enthalpy transfer
        cloud.hsTrans()[this->cell()] += np0*dhsTrans;

        // Update sensible enthalpy transfer coefficient
        cloud.hsCoeff()[this->cell()] += np0*Sph;

        // Update mass transfer term
        cloud.rhoTrans()[this->cell()] += -np0*(mDNew-mD0);

        // Couple the source to the stochastic particles
        // =============================================

        const scalar hDot = -np0*(mDNew*hA_new - mD0*hA_old)/dt;
        const scalar mDot = -np0*(mDNew-mD0)/dt;
...
template<class ParcelType>
template<class TrackCloudType>
void Foam::DropletSprayDNSThermoParcel<ParcelType>::timeIntegrationEulerExplicit
(
    TrackCloudType& cloud,
    trackingData& td,
    scalar& muRef,              // Viscosity at reference conditions
...
template<class ParcelType>
template<class TrackCloudType>
void Foam::DropletSprayDNSThermoParcel<ParcelType>::timeIntegrationCrankNicolson
(
    TrackCloudType& cloud,
    trackingData& td,
    scalar& muRef,              // Viscosity at reference conditions
...
Foam::DropletSprayDNSThermoParcel<ParcelType>::matrixSolver2By2
(
    const scalar a11,
    const scalar a12,
    const scalar a21,
...
template<class ParcelType>
template<class TrackCloudType>
void Foam::DropletSprayDNSThermoParcel<ParcelType>::mDotAndQDot
(
    TrackCloudType& cloud,
    trackingData& td,
...
Foam::DropletSprayDNSThermoParcel<ParcelType>::YtoX
(
    const List<scalar>& Y,
...
template<class ParcelType>
Foam::List<Foam::scalar> 
Foam::DropletSprayDNSThermoParcel<ParcelType>::XtoY
(
    const List<scalar>& X,
    const List<scalar>& MW
) const
...
void Foam::DropletSprayDNSThermoParcel<ParticleType>::initStatisticalSampling()
{
...
Foam::DropletSprayDNSThermoParcel<ParticleType>::getStatisticalData
(
    const wordList& vars
) const
{
...
Foam::DropletSprayDNSThermoParcel<ParticleType>::getStatisticalDataNames
(
    const wordList& vars
) const
{
...
#include "DropletSprayDNSThermoParcelIO.C"
