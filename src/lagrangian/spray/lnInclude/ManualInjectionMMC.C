...
#include "ManualInjectionMMC.H"
#include "mathematicalConstants.H"
#include "bitSet.H"
#include "volFields.H" ////Change 20240824
#include "fvMesh.H"     ///Change 20240824
#include "dictionary.H" ///Change 20240824  
#include "IOobject.H"   ///Change 20240824
// #include "interpolationCellPoint.H" ///Change 20240824
...
Foam::ManualInjectionMMC<CloudType>::ManualInjectionMMC
(
...
:
    ...
    UFieldName_(this->coeffDict().getOrDefault(Foam::word("UName"),Foam::word("U"))),
    UField_(owner.db().objectRegistry::lookupObject<volVectorField>(UFieldName_)), 
...
Foam::ManualInjectionMMC<CloudType>::ManualInjectionMMC
(
    const ManualInjectionMMC<CloudType>& im
)
:
    InjectionModel<CloudType>(im),
    UFieldName_(im.UFieldName_),
    UField_(im.UField_),                    
...
template<class CloudType>
void Foam::ManualInjectionMMC<CloudType>::updateMesh()
{
    label nRejected = 0;
...
template<class CloudType>
Foam::scalar Foam::ManualInjectionMMC<CloudType>::timeEnd() const
{
...
template<class CloudType>
Foam::label Foam::ManualInjectionMMC<CloudType>::parcelsToInject
(
    const scalar time0,
    const scalar time1
)
...
template<class CloudType>
Foam::scalar Foam::ManualInjectionMMC<CloudType>::volumeToInject
(
    const scalar time0,
    const scalar time1
)
...
template<class CloudType>
void Foam::ManualInjectionMMC<CloudType>::setPositionAndCell
(
    const label parcelI,
    const label,
    const scalar,
...
template<class CloudType>
void Foam::ManualInjectionMMC<CloudType>::setProperties
(
    const label parcelI,
    const label,
    const scalar,
...
{    
    label cellIndex = injectorCells_[parcelI];

    vector cellVelocity = UField_[cellIndex];

    // set particle velocity
    parcel.U() = cellVelocity;
...
template<class CloudType>
bool Foam::ManualInjectionMMC<CloudType>::fullyDescribed() const
{
    return false;
}
...
template<class CloudType>
bool Foam::ManualInjectionMMC<CloudType>::validInjection(const label)
{
    return true;
}
