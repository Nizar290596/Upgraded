...
Description
    Set values of the field to a flamelet solution through a dictionary.
...
//    PtrList<volScalarField>& Yinit = composition.Y();


    //- read dictionary
    Info<< "Reading setFlameletsDict\n" << endl;

    IOdictionary setFlameletsDict
    (
        IOobject
        (
            "setFlameletsDict",
    ...
    if (setFlameletsDict.found("defaultFieldValues"))
    {
        Info<< "Setting field default values" << endl;
        PtrList<setCellField> defaultFieldValues
        (
            setFlameletsDict.lookup("defaultFieldValues"),
    ...
    PtrList<entry> regions(setFlameletsDict.lookup("regions"));
