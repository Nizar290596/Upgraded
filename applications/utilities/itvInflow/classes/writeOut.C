...
writeOut::writeOut(const volVectorField& U_,const fvMesh & mesh_,const Foam::Time &runTime_,const polyPatch& patch,const label& cells)
:
U(U_),
mesh(mesh_),
runTime(runTime_)
...
fvPatchField<tensor> writeOut::redToPatch(const polyPatch& patch,label cells){        // averages extruded domain to a patch
...
void writeOut::writeOutVarPatch(const fvPatchField<tensor>& patch){
...
void writeOut::writeOutMeanPatch(const fvPatchField<vector>& patch){
...
void writeOut::redToLine(fvPatchField<tensor> varPatch)    // reduces a patch to a line by averaging over phi
{
