...
TEST_CASE("DropletSprayThermoParcel-Test","[sprayParcels][sprayParcels-parallel]")
{
    ...
    writeThermophysicalPropertiesDict(runTime);
    writeCloudProperties(runTime);
    writeSprayCloudProperties_EulerExplicit_OFliquidProperties(runTime);
    writeChemistryProperties(runTime);

    // END of all necessary fields for mmcCloud

    SECTION("Construtctor")
    {
        INFO("Create DropletSprayThermoParcel");
        ...
        // Create a particle
        basicDropletSprayThermoParcel p
        (
            mesh,
        ...
        SECTION("Particle IO - ASCII")
        {
            if (!Pstream::parRun())
            {
                ...
                OFstream os("DropletSprayThermoParcel.txt",OFstream::streamFormat::ASCII);
                os << p;
                os << flush;
                // Read from file 
                IFstream is("DropletSprayThermoParcel.txt",OFstream::streamFormat::ASCII);
                basicDropletSprayThermoParcel pNew(mesh, is);
        
                REQUIRE(pNew.T() == p.T());
                REQUIRE(pNew.Cp() == p.Cp());
                REQUIRE(pNew.rho() == p.rho());
                REQUIRE(pNew.mass0() == p.mass0());
            }
        }
            ...
            if (Pstream::parRun())
            {
                ...
                for (label procI=0; procI < Pstream::nProcs(); procI++)
                {
                    if (procI != Pstream::myProcNo())
                    {
                        ...
                        basicDropletSprayThermoParcel pNew(mesh,fromBuffer);
                        REQUIRE(pNew.T() == p.T());
                        REQUIRE(pNew.Cp() == p.Cp());
                        REQUIRE(pNew.rho() == p.rho());
                        REQUIRE(pNew.mass0() == p.mass0());
...
TEST_CASE("DropletSprayThermoParcel-Test ethanol droplet","[sprayParcels]")
{
    ...
    writeVolFields(mesh,1E+5,400,vector(2,0,0));
    ...
    {
        writeSprayCloudProperties_EulerExplicit(runTime);
        ...
        REQUIRE_THAT(sprayCloud.liquidProperties().Df(101325,298,28.96),Catch::Matchers::WithinAbs(1.35E-6,0.15E-6));
        ...
        REQUIRE(sprayCloud.first()->d() == 6E-4);
        REQUIRE(sprayCloud.first()->T() == 293.15);
        ...
        std::ofstream of("dropletDiameter_Ethanol_EulerExplicit.dat");
    ...
    SECTION("EulerExplicit OFFluidProperties")
    {
        ...
        // OpenFOAM uses a different function to calculate Df
        REQUIRE_THAT(sprayCloud.liquidProperties().Df(101325,298,28.96),Catch::Matchers::WithinAbs(8E-6,0.15E-6));
        ...
        // Write the droplet diameter to a file 
        std::ofstream of("dropletDiameter_Ethanol_EulerExplicit_OFLiquid.dat");
        for (auto& e : timeEvolutionDiameter)
            of << e[0]<<"\t"<<e[1]<<"\t"<<e[2]<<std::endl;
        of.close();
    ...
    {
        writeSprayCloudProperties_CrankNicolson(runTime);
        ...
        // Write the droplet diameter to a file 
        std::ofstream of("dropletDiameter_Ethanol_CrankNicolson.dat");
        for (auto& e : timeEvolutionDiameter)
            of << e[0]<<"\t"<<e[1]<<"\t"<<e[2]<<std::endl;
        of.close();
...
void writeThermophysicalPropertiesDict_nHeptane(Foam::Time& runTime)
{
    IStringStream is
    (
        "thermoType"
...
void writeSprayCloudProperties_EulerExplicit(Foam::Time& runTime)
{
    ...
    (
        ...
        "{"
            ...
            "           #};\n"
            "       }"
            "       binaryDiffusionCoefficient  5.74e-06;\n"
            "\n"
            "       latentHeat\n"
            "       {\n"
            "           type    coded;\n"
